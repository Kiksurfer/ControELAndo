...
LETTERS_ES = {
    ...
    "i": "I", "india": "I", "y": "I",  # Google suele transcribir 'i' como 'y'
    "j": "J", "jota": "J", "juliet": "J",
    "k": "K", "ka": "K", "kilo": "K",
    "l": "L", "ele": "L", "lima": "L",
    "m": "M", "eme": "M", "mike": "M",
...
def parse_coordinate(text):
    ...
    m = re.search(
        ...
        r"[a-o]|y|be|ce|se|de|ge|je|ka|pe|te)\s*(\d{1,2})\b",
        t,
    )
    if m:
        lw = m.group(1)
...
def act_open_keyboard():
    """
    Abre el teclado en pantalla. Prueba varios metodos en orden:
    1. TabTip.exe  — teclado tactil moderno de Windows 10/11
    2. osk.exe     — teclado clasico de accesibilidad (Sysnative y System32)
    3. Activacion por registro (metodo alternativo para TabTip)
    """
    import winreg

    # Metodo 1: TabTip.exe (teclado tactil moderno Windows 10/11)
    tabtip_paths = [
    ...
    for p in tabtip_paths:
        if os.path.exists(p):
            try:
                subprocess.Popen([p])
                print(f"[teclado] abierto via TabTip: {p}")
                return
            except Exception:
                pass

    # Metodo 2: osk.exe (teclado de accesibilidad clasico)
    osk_paths = [
    ...
    for p in osk_paths:
        try:
            subprocess.Popen([p])
            print(f"[teclado] abierto via osk: {p}")
            return
        except Exception:
            continue

    # Metodo 3: activar TabTip via registro
    try:
        key = winreg.OpenKey(
            winreg.HKEY_LOCAL_MACHINE,
            r"SOFTWARE\Microsoft\Windows NT\CurrentVersion\Accessibility",
            0, winreg.KEY_SET_VALUE)
        winreg.SetValueEx(key, "Configuration", 0, winreg.REG_SZ,
                          "ShowTabletKeyboard")
        winreg.CloseKey(key)
        pyautogui.hotkey("win", "ctrl", "o")
        print("[teclado] activado via registro")
    except Exception as e:
        print(f"[teclado] error: {e}")
        # Ultimo recurso: atajo de teclado Win+. o Win+Ctrl+O
        try:
            pyautogui.hotkey("win", "ctrl", "o")
        except Exception:
            pass
...
def act_close_keyboard():
    """Cierra tanto osk.exe como TabTip.exe (teclado moderno Windows 10/11)."""
    for proc in ("osk.exe", "TabTip.exe", "tabtip.exe"):
        try:
...
class GridOverlay:
    ...
    def _draw_zoom(self):
        ...
        sw = self.cell_w / 2
        sh = self.cell_h / 2
        parent_label = f"{LETTERS[col]}{row + 1}"
    ...
    def zoom_cell(self, col, row):
        ...
        if not self.visible:
            self.show()
        print(f"[zoom] celda {LETTERS[col]}{row + 1}")
...
class HudOverlay:
    ...
    def __init__(self, master):
        ...
        top.pack(fill="x")
        self.dot_canvas = tk.Canvas(top, width=14, height=14,
        ...
        # Fila inferior: lo que acaba de escuchar (siempre visible)
        self.lbl_heard = tk.Label(frm, text="",
                                  fg="#888888", bg="#101010",
                                  font=("Segoe UI", 8, "italic"),
                                  anchor="w")
        self.lbl_heard.pack(fill="x", pady=(1, 0))
...
class ActionBar:
    ...
    def __init__(self, master, controller, cfg):
        ...
        btn(self.full_frame, "Teclado", self.ctrl.toggle_keyboard
            ).pack(side="left", padx=2, pady=3)
...
class SuperCapa:
    ...
    def _handle_dictation(self, text):
        ...
        if any(t in text for t in INTENTS["DICTATE_CLEAR"]):
            ...
            return
        # Mostrar en el HUD lo que se va a escribir
